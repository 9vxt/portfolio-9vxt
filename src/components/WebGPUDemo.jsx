...
fn main(@builtin(global_invocation_id) id : vec3<u32>) {
  ...
  var p = particles[i];
  p.vel.x += (sin(p.pos.y * 0.5 + time) - p.pos.x * 0.1) * 0.003;
  p.vel.y += (cos(p.pos.x * 0.5 + time * 0.7) - p.pos.y * 0.1) * 0.003;
  p.pos += p.vel;
...
const renderVert = `
struct VertexOut { @builtin(position) pos : vec4<f32>, @location(0) col : vec3<f32> }
@vertex fn main(@builtin(vertex_index) vi : u32) -> VertexOut {
  let i = vi / 4u;
  let vert = vi % 4u;
  let px = f32(i % 64u) / 64.0 * 2.6 - 1.3;
  let py = f32(i / 64u) / 48.0 * 2.0 - 1.0;
  let size = 0.012;
  var out: VertexOut;
  if (vert == 0u) { out.pos = vec4(px - size, py - size, 0.0, 1.0); }
  else if (vert == 1u) { out.pos = vec4(px + size, py - size, 0.0, 1.0); }
  else if (vert == 2u) { out.pos = vec4(px + size, py + size, 0.0, 1.0); }
  else { out.pos = vec4(px - size, py + size, 0.0, 1.0); }
  out.col = vec3(0.23, 0.51, 0.96);
  return out;
}`

const renderFrag = `
@fragment fn main(@location(0) col : vec3<f32>) -> @location(0) vec4<f32> {
  return vec4(col, 0.6);
}`
...
async function initWebGPU(canvas, particleCount) {
  ...
  const compModule = device.createShaderModule({ code: shaderCode })
  ...
  const renderModule = device.createShaderModule({ code: renderVert + renderFrag })
  const renderPipe = device.createRenderPipeline({
    layout: 'auto',
    vertex: { module: renderModule, entryPoint: 'main' },
    fragment: { module: renderModule, entryPoint: 'main', targets: [{ format: 'bgra8unorm', blend: { color: { srcFactor: 'src-alpha', dstFactor: 'one', operation: 'add' }, alpha: { srcFactor: 'one', dstFactor: 'one', operation: 'add' } } }] },
...
export default function WebGPUDemo() {
  ...
  useEffect(() => {
    ...
    initWebGPU(canvas, 3072).then((cleanup) => {
      ...
      if (e.message !== 'WebGPU not supported') setError(e.message)
      setSupported(false)
    })
    ...
    <section id="gpu" className="py-24 bg-[#080c14] border-t border-[#1e293b]">
      <div ref={ref} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <canvas ref={canvasRef} width={512} height={320} className="w-full h-auto rounded bg-black" style={{ imageRendering: 'pixelated', aspectRatio: '512/320' }} />
