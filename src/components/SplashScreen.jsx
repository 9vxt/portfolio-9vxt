...
export default function SplashScreen({ onFinish }) {
  ...
  useEffect(() => {
    ...
    const fallback = setTimeout(handler, 6000)
    return () => { clearInterval(id); clearTimeout(fallback); window.removeEventListener('keydown', handler); window.removeEventListener('click', handler) }
  }, [done, onFinish])

  return (
    <div
      className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#080c14] transition-opacity duration-600 ${fading ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
